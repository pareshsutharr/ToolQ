...
export default function ExtractPagesPage() {
  ...
  async function extract() {
    ...
    if (indices.length === 0) {
      setError(`Enter valid page numbers to keep, e.g. 1-3,5 (this PDF has ${pageCount} pages).`);
      return;
    }
    setBusy(true);
    setError(null);
    try {
      ...
      setResult({ name: "extracted.pdf", url: URL.createObjectURL(blob), size: blob.size });
    } catch {
      setError("Couldn't extract those pages.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <ToolShell title="Extract Pages" description="Pull specific pages out into a new PDF.">
      {result ? (
        ...
        <div className="flex flex-col gap-4">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-ink/70">Pages to keep</label>
            <input
              value={ranges}
